...
function pdf2html($html_file_dest,$unique_filename,$file,$width,$ext) {
     ...
     shell_exec('/usr/bin/php trial.php '.$unique_filename.' '.$u.' '.$ext.' > uploaded/tt.txt 2>&1 &');    
        	...
        	// $cmd = "pdf2htmlEX --process-outline 0 --fit-width 800 --fit-height 1200 --dest-dir " . $html_file_dest . " uploaded/uploaded_files_".$_SESSION['userid'] . "_original/".$unique_filename.".pdf 2>&1 &"; 
   	        ...
   	        $cmd = "pdf2htmlEX --process-outline 0 --hdpi 100 --vdpi 100 --tounicode 1 --decompose-ligature 1 --fit-width ".$width." --dest-dir " . $html_file_dest . " uploaded/uploaded_files_".$_SESSION['userid'] . "_original/".$unique_filename.".pdf 2>&1 &"; //>>dta.log helps run the cmd even when a large amnt of data is printed on cmd page
...
//  $pathToFile = $html_file_dest.'/'.$unique_filename.'.html';

//             $s3->putObject(array(
//     'Bucket' => 'docs-ent',
//     'Key'    => 'uploaded/user_'. $_SESSION["userid"].'/converts/'.$unique_filename.'.html',
//     'Body'   => EntityBody::factory(fopen($pathToFile, 'w+')),
...
//         			$pdf_file   = 'uploaded/uploaded_files_' . $_SESSION['userid'] . '_original/' . $unique_filename.'.pdf';
...
// $bucket = 'docs-ent';
// $keyname = 'uploaded/user_'. $_SESSION["userid"].'/thumbnails/'.$unique_filename.'.jpg';
...
// $bucket = 'docs-ent';
// $keyname = 'uploaded/user_'. $_SESSION["userid"].'/original/'.$unique_filename.'.pdf';
